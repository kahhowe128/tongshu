...
const ACADEMY = [
  ...
  {
    id: 'openshop', figure: 'openshop', readMin: 5,
    ...
    sections: [
      { h: ['为什么开市要择日', 'Why timing a launch matters'],
        body: ['「开市」在通书里是一桩大事——它被看作财气的起点，传统上与「交易」「立券」「纳财」同属商业类。择日的用意，并非迷信地相信某天会自动带来生意，而是借一套两千年沉淀的规则，避开公认的凶日、择取公认的吉日，让开张这天「天时」无碍、人心安定。清代《協紀辨方書》把宜忌系统化，正是为这类大事提供可核对的依据。\n\n但要诚实地说：现代经营的成败，取决于选址、产品与服务，远多于黄历。择日是锦上添花的传统仪式，给人信心与好兆头，而非商业计划的替代。本应用因此只如实呈现传统结论与其依据，由你自行权衡。',
          'In the almanac, "opening for business" (开市) is a weighty affair — read as the start of a venture\'s fortune, and grouped with trade, signing contracts and receiving wealth. The point of date-selection is not a superstitious belief that a day will conjure customers, but to lean on two millennia of codified rules to avoid the broadly-agreed ill days and take the agreed-auspicious ones, so the launch meets no "heavenly" obstruction and minds are at ease. The Qing-era Xieji Bianfang Shu systematised these 宜/忌 precisely to give such occasions a checkable basis.\n\nIn honesty, though: a modern business rises or falls far more on location, product and service than on the almanac. Date-selection is a grace-note ritual that lends confidence and a good omen — not a substitute for a business plan. So this app simply shows the traditional verdict and its reasons, and leaves the weighing to you.'] },
      { h: ['第一步 — 定事项与范围', 'Step 1 — set the activity and window'],
        body: ['打开「黄历／择日」，在事项里选「开市／开业」。若你同时要签约、进货，也一并勾选「立券」「交易」——工具会综合各事项给出当天的总结论，而非只看一项。\n\n接着定范围。开店通常有装修、备货的弹性，建议给一到两个月：候选越多，挑到「既吉、又不冲你」的日子机会越大。范围太窄（例如只剩三天）往往无好日可选，反而被迫将就。定好后，工具会按宜忌为范围内每一天打分、排序，把高分日浮到前面。',
          'Open Calendar / Find and choose the activity 开市 (open business). If you will also sign a lease or take delivery of stock, tick 立券 (sign contract) and 交易 (trade) too — the tool blends all chosen activities into one verdict for the day, rather than judging a single one.\n\nThen set the window. A shop fit-out usually has slack, so give it one to two months: the more candidates, the better your odds of a day that is both auspicious and does not clash you. Too narrow a window (say three days left) often leaves no good day and forces a compromise. Once set, the tool scores and ranks every day in range, floating the strong ones to the top.'] },
      { h: ['第二步 — 读懂一个高分日', 'Step 2 — read a high-scoring day'], figure: 'openshop_read',
        body: ['点开一个高分日，你会看到几条关键信息，逐一对照：\n\n建除神：十二值日神轮流当班。「开」日利开张、「成」日诸事易成、「定」日安稳，皆利商；「破」「危」日则多忌，开市尤须避「破」。\n\n黄道／黑道：看当值的是黄道六神（吉）还是黑道六神（凶），开市偏好黄道日。\n\n二十八宿：某些宿传统视为吉，应用会标出当日宿与其吉凶倾向。\n\n神煞：当日的吉神（如天德、月德）加分，凶煞（如月破、受死）减分；应用只采高、中置信者计分，版本互斥者明示「未纳入」，宁缺毋造。\n\n把这几条一起读，你就明白这天「为什么是高分」，而不是盲信一个数字。',
          'Open a high-scoring day and you will see a few key lines to check off, one by one:\n\nThe day-officer (建除): twelve officers rotate on duty. An "Open" (开) day favours launches, a "Complete" (成) day lets matters succeed, a "Stable" (定) day is settled — all good for commerce; "Break" (破) and "Danger" (危) days carry many taboos, and an opening especially must avoid 破.\n\nBright vs dark path (黄道／黑道): whether a bright (auspicious) or dark (inauspicious) path-god holds the day; openings prefer a bright-path day.\n\nThe lunar mansion (宿): some of the 28 mansions are traditionally auspicious; the app marks the day\'s mansion and its leaning.\n\nThe spirits (神煞): benefics on the day (e.g. Tiande, Yuede) add, malefics (e.g. Yuepo, Shousi) subtract; the app scores only high- and medium-confidence ones and openly marks conflicting versions as omitted rather than inventing them.\n\nRead these together and you understand why a day scores high — instead of trusting a bare number.'] },
      { h: ['第三步 — 避开冲煞', 'Step 3 — avoid the clash'], figure: 'zodiacclash',
        body: ['一个客观上的吉日，仍可能「冲」到你。通书每日标「冲某煞某方」：那天地支所冲的生肖，传统上当事人应回避大事。\n\n开店的店主（或主事者）若生肖正被这天所冲，多半会另择一天，哪怕分数略低。在设定里填入你的生肖，应用就会替你把「冲你」的日子标出来，并指出煞方。\n\n记住：冲是提醒而非判决，你可结合分数与现实权衡；但传统上，店主本命被冲的开张日，宁可避之。',
          'A day that is auspicious in the abstract may still "clash" you. Each almanac day is marked "clashes sign X, ill direction Y": the zodiac sign opposed that day should, by tradition, avoid major undertakings.\n\nIf the shop\'s owner (or the person in charge) is the sign clashed that day, most would pick another day even at a slightly lower score. Set your zodiac in Settings and the app will mark the days that clash you, and name the ill direction.\n\nRemember: a clash is a flag, not a sentence; weigh it against the score and reality. But traditionally, an opening day that clashes the owner\'s own sign is best avoided.'] },
      { h: ['第四步 — 挑一个吉时开门', 'Step 4 — pick the hour to open'], figure: 'hours',
        body: ['选定吉日后，还可「精挑」当日的吉时。一天分十二时辰，各有黄道黑道与所冲；开市讲究「开门」那一刻的吉。\n\n传统上，许多店家选当日的黄道吉时、且不冲店主生肖的时辰举行开张仪式——剪彩、放鞭炮、迎第一单。应用为每天列出十二时辰，以「宜／平／忌」标示，凶险时辰绝不标为宜；你可挑一个「宜」且不冲的时辰落定。\n\n若吉日难寻，吉时便是退而求其次的「补救」：一个好时辰，能为一个平常的日子添几分从容。',
          'With the day chosen, you can fine-tune the hour. A day splits into twelve double-hours, each with its own bright/dark path and clash; an opening cares about the auspiciousness of the very moment the doors swing open.\n\nTraditionally, many shopkeepers hold the ceremony — ribbon, firecrackers, the first sale — in a bright, non-clashing double-hour. The app lists all twelve per day, graded favourable / neutral / avoid, and never marks a dangerous hour favourable; pick one that is favourable and does not clash.\n\nAnd when a perfect day is scarce, the hour is the fall-back remedy: a good hour lends a little grace to an ordinary day.'] },
    ],
    cases: [
    ...
    glossaryLinks: ['jianchu', 'huangdao', 'ss', 'verdict'],
  },
];
